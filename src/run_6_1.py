...
# -*- coding: utf-8 -*-
"""
run_6_1.py — Generate dataset *and* run §6‑1 Static+Scalability benchmark
==========================================================================
...
import argparse, math, random, datetime, json, shutil, subprocess, sys, os, time
...
def build_dataset(dst_root: Path, step: int):
    ...
    for n in range(10, 101, step):
        ...
        pid += 1
    # 60N / 120F static case  (若不在 sweep範圍則另補)
...
def main():
    ...
    # 呼叫 main_exp.py 作為子行程 (保持單一程式碼來源)
    cmd = ["python3", "main_exp.py",
           "--alg", args.alg,
           "--path", str(base/"data"),
           "--ins", "0",
           "--ga_workers", str(args.workers),
           "--out", str(base/"results")]
    # main_exp 的 --start/--end 預設跑整份 ; 若需調整可在此增參
    ...
    # 匯總並畫圖
    all_csv = base/"results"/"all_results_0_0_"+str(len(pd.read_csv(data_dir/"dataset_logs.csv"))-1)+".csv"
    if all_csv.is_file():
