...
def RTAS2018(task_path,
    ...
    try:
        ...
        z3.set_param('smt.random_seed', 2)
        ...
        for link, link_vars in net_var.items():
            phi_vals = [m.evaluate(link_vars['phi'][i]).as_long() for i in range(net_attr[link]['W'])]
            tau_vals = [m.evaluate(link_vars['tau'][i]).as_long() for i in range(net_attr[link]['W'])]
            k_vals = [m.evaluate(link_vars['k'][i]).as_long() for i in range(net_attr[link]['W'])]
            print(f"Link {link}: phi={phi_vals}, tau={tau_vals}, k={k_vals}")

        # 打印task_var中的值
        for task, task_links in task_var.items():
            for link, windows in task_links.items():
                window_vals = [m.evaluate(w).as_long() for w in windows]
                print(f"Task {task}, Link {link}: windows={window_vals}")
