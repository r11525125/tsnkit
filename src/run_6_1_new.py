...
import argparse, math, random, datetime, subprocess, sys
...
PERIOD=(1_000_000,2_000_000); SIZE=(256,1024)
...
def build_dataset(root:Path,step:int)->Path:
    ...
    d0=root/"data"/"0"; d0.mkdir(parents=True,exist_ok=True)
    rnd=random.Random(2025)         # fixed seed
    logs=[]; pid=0
    for n in range(10,101,step):
        e=_gen_one(pid,n,2*n,d0,rnd); logs.append([pid,n,2*n,e]); pid+=1
    if 60 not in range(10,101,step):
        e=_gen_one(pid,60,120,d0,rnd); logs.append([pid,60,120,e])
...
def quick_plot(df:pd.DataFrame,out:Path):
    ...
    for m,ylab,fn in [("solve_time","Solve‑Time (s)","curve_time.png"),
        ...
        plt.savefig(out/fn,dpi=300); plt.close()
...
def main():
    ...
    # ----- invoke main_exp2.py ---------------------------------------------
    cmd=[sys.executable,"main_exp2.py",
         "--alg",A.alg,
         "--path",str(root/"data"),
         "--ins","0",
